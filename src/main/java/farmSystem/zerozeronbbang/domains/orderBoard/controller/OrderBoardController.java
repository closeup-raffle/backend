...
public class OrderBoardController {
    ...
    public ResponseDto<Page<ResFindOrderBoardsDto>> getOrderBoards(@AuthenticationPrincipal final String email, @PageableDefault(size=10, sort = "createdAt", direction = Sort.Direction.DESC)Pageable pageable){
        Page<ResFindOrderBoardsDto> orderBoards = orderBoardService.findOrderBoards(pageable);
        return ResponseUtil.SUCCESS(ResCodeEnum.ORDERBOARD_LOAD_SUCCESS.getMessage(), orderBoards);
    }

    //@Authen~~에서 email 가져오는 부분이 안됨. ㄴervice에서 user 조횝 부분 null값 뜸
    @Operation(summary = "orderBoard list", description = "orderBoard 리스트 조회 컨트롤러")
    @PostMapping(value = "/order-board")
    public ResponseDto<?> writeOrderBoard(@AuthenticationPrincipal final String email, @RequestBody ReqWriteOrderBoardDto reqWriteOrderBoardDto){
        return ResponseUtil.SUCCESS(ResCodeEnum.ORDERBOARD_WRITE_SUCCESS.getMessage(),orderBoardService.writeOrderBoard(email, reqWriteOrderBoardDto));
    }
